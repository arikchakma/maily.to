...
import { findElementNextToCoords } from './helpers/find-next-element-from-cursor';
import { getOuterNode, getOuterNodePos } from './helpers/get-outer-node';
import { removeNode } from './helpers/remove-node';
...
const getAbsolutePos = (state: EditorState, relativePos: number) => {
  const ystate = ySyncPluginKey.getState(state);
...
export const dragHandlePluginDefaultKey: PluginKey = new PluginKey(
  'dragHandle'
);
...
export const DragHandlePlugin = ({
  ...
  function repositionDragHandle(dom: Element) {
    ...
    void computePosition(virtualElement, element, computePositionConfig).then(
      (val) => {
    ...
    plugin: new Plugin({
      ...
      view: (view) => {
        ...
        const handleResize = () => {
          ...
          const outerDomNode = getOuterDomNode(view, domNode);
          if (outerDomNode === view.dom) {
            return;
          }

          repositionDragHandle(outerDomNode);
        };
        ...
        return {
          update(_, oldState) {
            ...
            // Recalculate popup position if doc has changed and drag handler is visible.
            ...
            const isSameNode =
              currentNode?.type === outerNode?.type &&

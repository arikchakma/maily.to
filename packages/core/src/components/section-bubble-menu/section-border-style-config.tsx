...
type SectionBorderStyleConfigProps = {
  container: FloatingUIContainer;
  editor: Editor;
  onBack: () => void;
};

export function SectionBorderStyleConfig(props: SectionBorderStyleConfigProps) {
  ...
  const handleWidthModeChange = (nextWidthMode: AllowedFieldMode) => {
    editorInstance
      .chain()
      .updateSection({ borderWidthMode: nextWidthMode })
      .run();
  };
  ...
  const handleRadiusModeChange = (nextRadiusMode: AllowedFieldMode) => {
    editorInstance
      .chain()
      .updateSection({ borderRadiusMode: nextRadiusMode })
      .run();
  };
  ...
  const handleBorderStyleChange = (nextBorderStyle: AllowedBorderStyle) => {
    editorInstance
      .chain()
      .updateSection({ borderStyle: nextBorderStyle })
      .run();
  };
  ...
  return (
    <div className="mly:flex mly:flex-col mly:gap-1">
      ...
      <MultiUnitField
        ...
        mode={state.borderRadiusMode as AllowedFieldMode}
        onModeChange={handleRadiusModeChange}
        values={{
          topLeft: state.borderTopLeftRadius,
          topRight: state.borderTopRightRadius,
          bottomRight: state.borderBottomRightRadius,
          bottomLeft: state.borderBottomLeftRadius,
        }}
      ...
      {state.borderRadiusMode === FIELD_MODE.MIXED && (
        <Divider type="horizontal" className="mly:-mx-1" />
      )}
      ...
      <MultiUnitField
        ...
        container={container}
        mode={state.borderWidthMode as AllowedFieldMode}
        onModeChange={handleWidthModeChange}
        values={{
          top: state.borderTopWidth,
          right: state.borderRightWidth,
          bottom: state.borderBottomWidth,
          left: state.borderLeftWidth,
        }}
      ...
      {state.borderWidthMode === FIELD_MODE.MIXED && (
        <Divider type="horizontal" className="mly:-mx-1" />
      )}
